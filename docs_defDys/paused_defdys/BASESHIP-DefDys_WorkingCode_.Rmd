...
rtData <- readtext("C:/Users/white/iCloudDrive/Documents/1_DACSS_UMassAmherst/Dacss_RProjects/3. FL21_rProjects/602_soloGroupProject/em_defDys_textAnalysis/data_defDys/defDys_roundUp.csv", 
                   text_field = "Abstract Note")

absData<- rtData

absData <- select(absData,c(1,2,3,4,5,6,7,8,28,30,41))
      colnames(absData)
      view(summary(absData))

absData %>% distinct("Title", .keep_all = TRUE)
      summary(absData)

absUniq <- unique.data.frame(absData)
        summary(absUniq)


absCorp <- corpus(absData)
        summary(absData)
        view(head(absCorp,10))


absCorp %>% corpus(absCorp,
                  unique_docnames = FALSE,
                   docid_field = "Key",
                   docnames = quanteda::docnames(absCorp"Title"))
...
Title Abstract.Note Manual.Tags Automatic.Tags
Item.Type Publication.Title Publication.Year
...
refDoc_title refDoc_absNote refDoc_manTags
refDoc_autoTags refDoc_itemType refDoc_pubTitle
refDoc_pubYear
...
absDoc_title absDoc_absNote absDoc_manTags
absDoc_autoTags absDoc_itemType absDoc_pubTitle
absDoc_pubYear absDoc_libCat
...
s_tle_refCorpSub returns: 11 s_mTag_refCorpSub
returns: 1

    Signal 
      absCorpSub Group

s_tle_absCorpSub returns: 0 s_mTag\_\_absCorpSub
returns: 0

    Noise
      refCorpSub Group

n_tle_refCorpSub returns: 14 n_MTag_refCorpSub
returns: 1

    Noise
      absCorp Group

n_tle_absCorpSub returns: 7 n_mTag_absCorpSub
returns: 0
...
# (*) NOTE: I saw chunk options " Tokenize: AbsCorp & RefCorp eval=FALSE, include=FALSE"
...
Docvars Categories Title Abstract.Note Manual.Tags
Automatic.Tags Item.Type Publication.Title
...
title_refTok absNote_refTok manTag_refTok
itmTyp_refTok pubTle_refTok
...
title_absTok absNote_absTok manTag_absTok
itmTyp_absTok \# cent_absTok
...
Signal: Abstract Note sUto_aNote_refKwt
sDyo_aNote_refKwt

Signal: Manual Tags sUto_mTags_refKwt
sDyo_mTags_refKwt


```{r}
# Manual Tags 
sUto_mTags_refKwt <- kwic(manTags_refTok,pattern =
c("Dystopia","Dystopias",
"Dystopian","Dystopic","Anti-Utopian","AntiUtopian",
"Utopia","Utopias", "Utopian","Utopianism") )

sDyo_mTags_refKwt <- kwic(manTags_refTok,pattern
= c("Dystopia","Dystopias",
"Dystopian","Dystopic","Anti-Utopian","AntiUtopian",
"Utopia","Utopias", "Utopian","Utopianism") )
...
sUti_mTags_refKwt \<- kwic(manTags_refTok, pattern
= "utopi\*", )

sDyi_mTags_refKwt \<- kwic(manTags_refTok, pattern
= "dystopi\*", )

Signal: Wildcards: Titles sUti_tle_refKwt
sDyi_tle_refKwt

Signal: Wildcards: Abstract Note sUti_aNote_refKwt
sDyi_aNote_refKwt

Signal: Wildcards: Manual Tags sUti_mTags_refKwt
sDyi_mTags_refKwt
...
Signal: Wildcards: Titles sUti_tle_absKwt
sDyi_tle_absKwt

Signal: Wildcards: Abstract Note sUti_aNote_absKwt
sDyi_aNote_absKwt

Signal: Wildcards: Manual Tags sUti_mTags_absKwt
sDyi_mTags_absKwt
...
Noise: Titles nApo_tle_refKwt nTop_tle_refKwt
nSub_tle_refKwt

Noise: Abstract Note nApo_aNote_refKwt
nSub_aNote_refKwt nTop_aNote_refKwt

Noise: Manual Tags nApo_mTag_refKwt
nSub_mTag_refKwt nTop_mTag_refKwt
...
```{r}
Manual Tags nApo_mTag_absKwt \<- kwic(
...
Noise: Abstract Note nApo_aNote_absKwt
nSub_aNote_absKwt nTop_aNote_refKwt

Noise: Manual Tags nApo_mTag_absKwt
nSub_mTag_absKwt nTop_mTag_absKwt
```
...
# Corpa
  tmTestCorp <- absDoc_absNote #Accepts Type: Character
...
choosing words that are the most `fr`equent *and*
`ex`clusive in each
...
# accepts Type: Character

refTle_corp <- refDoc_title
  
refDfm_titles <- tokens(refTle_corp,
      remove_punct = T,
      remove_symbols = T) %>%
    tokens_remove(
      stopwords("en"), 
...
90 documents: [ref] Dystopi\* + Utopi\* + Apo
Terms
...
398 documents: [ref] Dystopi\* + Utopi\* +
[ref+abs] Apo Terms
...
2864 documents: [abs] Dystopi\* + [ref+abs] Apo
Terms
